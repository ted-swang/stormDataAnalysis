...
```{r shrinkData, echo = TRUE, cache = TRUE}
...
```{r EVTYPETable, echo = TRUE, cache = TRUE}
...
```{r factorConsolidation, echo = TRUE}
...
We now summarize our data by finding the sum of all of our relevant categories for all years, `r min(stormData$Year)` to `r max(stormData$Year)`.
